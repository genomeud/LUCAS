...
tot<-rowSums(abs(cormat))
	cormat<-cormat[order(tot,decreasing=T),][1:25,]
corp2<-corp[row.names(corp)%in%row.names(cormat),]
cormat<-cormat[order(row.names(cormat)),]
corp2<-corp2[order(row.names(corp2)),]
col2 = colorRampPalette(c('darkred', 'white', 'blue3'))
pdf("megacorr_class2.pdf", width=10)

corrplot(cormat, p.mat = corp2, 
         tl.col="black", tl.cex=0.6,
         diag = TRUE, type = 'full',
         col=col2(256),
         insig = 'label_sig', sig.level = c(0.05), 
         title="Le venticinque specie più correlate alle proprietà del suolo",
